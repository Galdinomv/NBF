...
**It's done for 4 threads / 3 executions with 10 timesteps each
*/
...
int print(struct Thread TT[4][4]){
    int count=0;
    int test;
    int thread,transaction;
    
    for (transaction=0;transaction < 4 ; transaction++)
    {
...
struct Thread average(struct Thread TT[4][4])
{
    struct Thread avg[4][4];
    int count=0;
    int test;
    int thread,transaction;
